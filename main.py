...
def matrix_generator(max_y, min_y, len_matrix, mg):  # Генератор випадкових матриць для у
    ...
    for n in range(len_matrix):
        lis = [random.randrange(min_y, max_y) for _ in range(mg)]
        matrix.append(lis)
    return matrix
...
def fill_y_matrix_lab6(matr_x, lis_koef, m_f):
    matrix_f = []
    mkl = matr_x.copy()
    for k in range(m_f):
        liskl = []
        for i in mkl:
            lis = [el for el in i]
            lis.insert(0, 1)
            liskl.append(sum([x*y for x, y in zip(lis, lis_koef)])+random.randrange(10)-5)
        matrix_f.append(liskl)
